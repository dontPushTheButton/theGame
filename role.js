...
module.exports = {
	harvester: function (creep) {
		...
		if ((creep.carry.energy == 0 && creep.memory.offloading)) {
			...
			creep.memory.destination = creepPrimitives.findBestSource(creep); //bestSourceID;
			//console.log('test4');
		} else if (creep.room.propertyIsEnumerable("storage") && creep.memory.offloading && creep.room.energy === creep.room.energyCapacity && creep.room.storage.id !== creep.memory.destination) {
